...
export class Workspace {
  ...
  completeCurrentService() {
    ...
    this.toastr.success(`Completed service for ${this.currentTicket.customerName}`);
  ...
  callNextInQueue() {
    if (this.waitingQueue.length === 0) {
      this.toastr.warning('الطابور فاضي يا وحش، اشرب قهوتك!');
  ...
  callSpecificCustomer(ticketNumber: string) {
    ...
    if (index !== -1) {
      ...
      // this.toastr.info(`Now serving ${selectedCustomer.customer}`);
  ...
  skipCustomer() {
    ...
    this.toastr.info(`Skipped ${this.currentTicket.customerName}`);
  ...
  cancelTicket() {
    ...
    this.toastr.warning(`Canceled ticket ${this.currentTicket.number}`);
